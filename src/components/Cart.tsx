      ...
      <SheetContent className="flex w-full flex-col pr-0 sm:max-w-lg">
        ...
        {itemCount > 0 ? (
          ...
          <div className="flex h-full flex-col items-center justify-center space-y-1">
            <div className="relative mb-4 h-60 w-60 to-muted-foreground" aria-hidden='true'>
              <Image src='/hippo-empty-cart.png' fill alt="empty shopping cart" />
            </div>
            <div className="text-xl font-semibold">Your cart is empty</div>
            <SheetTrigger asChild>
              <Link
                className={buttonVariants({ variant: 'link', size: 'sm', className: 'text-sm to-muted-foreground' })}
                href='/products'>
                Explore  items
              </Link>
            </SheetTrigger>
