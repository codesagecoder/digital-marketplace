...
const Page = async ({ searchParams }: PageProps) => {
  ...
  const nextCookeis = cookies();

  const { user } = await getServerSideUser(nextCookeis);
            ...
            <div className="mt-16 text-sm font-medium">
              ...
              <ul className="mt-6 divide-y divide-gray-200 border-t border-gray-200 text-sm font-medium text-muted-foreground">
                {(order.products as Product[]).map(prod => {
                  const label = PRODUCT_CATEGORIES.find(({ value }) => value == prod.category)?.label;
                  const downloadUrl = (prod.product_files as ProductFile).url as string;
                  ...
                  return (
                    <li key={prod.id} className="flex space-x-6 py-6">
                      ...
                      <p className="flex-none font-medium  text-gray-900">{formatPrice(prod.price)}</p>
                    </li>
                  );
                })}
              </ul>
              <div className="space-y-6 border-t border-gray-200 pt-6 text-sm font-medium text-muted-foreground">
                ...
                <div className="flex items-center justify-between border-t border-gray-200 pt-6 to-gray-900">
